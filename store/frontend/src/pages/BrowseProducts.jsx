...
const BrowseProducts = () => {
  ...
  const fetchProducts = async () => {
    try {
      ...
      // Adapt to standard response or custom response depending on the backend
      // Assuming my backend returns { products: [...] } or the old UI { data: [...] }
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between h-16 items-center">
            <div className="flex-shrink-0 flex items-center">
              <h1 className="text-2xl font-extrabold text-slate-900 tracking-tight">Super<span className="text-indigo-600">Store</span></h1>
            </div>
            <div className="flex items-center space-x-6">
              <Link to="/cart" className="relative text-slate-600 hover:text-indigo-600 font-medium transition-colors flex items-center">
                Cart
                {cartCount > 0 && (
                  <span className="absolute -top-2 -right-4 bg-indigo-600 text-white text-xs font-bold rounded-full h-5 w-5 flex items-center justify-center shadow-sm">
                    {cartCount}
                  </span>
                )}
              </Link>
              {customer ? (
                <div className="flex items-center space-x-4">
                  <span className="text-indigo-600 font-semibold px-3 py-1 bg-indigo-50 rounded-full text-sm">
                    Hi, {customer.name}
                  </span>
                  <Link to="/orders" className="text-slate-600 hover:text-indigo-600 font-medium transition-colors">
                    My Orders
                  </Link>
                  <button onClick={logout} className="text-slate-600 hover:text-red-500 font-medium transition-colors">
                    Logout
                  </button>
                </div>
              ) : (
                <Link to="/login" className="bg-indigo-600 text-white px-5 py-2.5 rounded-full font-medium hover:bg-indigo-700 transition-colors shadow-sm hover:shadow-md text-sm">
                  Sign In
                </Link>
              )}
            </div>
          ...
          <div className="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden mb-12 relative mx-4 sm:mx-6 lg:mx-8 mt-6">
            ...
            <div className="relative pt-16 pb-20 px-6 sm:px-12 lg:px-20 text-center max-w-4xl mx-auto z-10">
              ...
              <button 
                ...
                className="bg-slate-900 hover:bg-slate-800 text-white font-medium py-3 px-8 rounded-full transition-colors shadow-md hover:shadow-lg"
              >
                Shop the Collection
              </button>
            </div>
                      ...
                      <div className="flex justify-between items-center mt-auto pt-4 border-t border-slate-100">
                        ...
                        {product.stockQuantity > 0 ? (
                          <button
                            ...
                            className="flex items-center justify-center bg-indigo-50 hover:bg-indigo-600 text-indigo-700 hover:text-white px-4 py-2 rounded-lg text-sm font-semibold transition-all duration-300 disabled:opacity-50"
                          >
                            {addingToCart === product._id ? 'Adding...' : 'Add to Cart'}
                          </button>
                        ) : (
                          <button disabled className="flex items-center justify-center bg-slate-100 text-slate-400 px-4 py-2 rounded-lg text-sm font-semibold cursor-not-allowed">
                            Out of Stock
                          </button>
                        )}
                      </div>
