...
const Register = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.post(`${API_URL}/customer-auth/register`, formData);
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50 py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-md w-full space-y-8 bg-white p-8 rounded-2xl shadow-xl transition-all duration-300 hover:shadow-2xl">
        <div>
        ...
        <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
          {error && (
            <div className="bg-red-50 text-red-500 p-3 rounded-lg text-sm text-center font-medium animate-pulse">
              {error}
            </div>
          )}
          <div className="rounded-md shadow-sm space-y-4">
            ...
            <div>
              ...
              <input id="address" name="address" type="text"
                className="appearance-none rounded-lg relative block w-full px-3 py-3 border border-gray-300 placeholder-gray-500 text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 focus:z-10 sm:text-sm transition-all"
                placeholder="Address (Optional)" value={formData.address} onChange={handleChange} />
            </div>
          </div>
