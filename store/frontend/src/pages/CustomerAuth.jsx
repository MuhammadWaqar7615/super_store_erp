...
const CustomerAuth = () => {
  ...
  // If navigated directly to /register or /auth?tab=register, start on the register tab
  useEffect(() => {
  ...
  const handleLoginSubmit = async (e) => {
    ...
    try {
      ...
      if (err.response?.data?.message === 'Please verify your account first.') {
        ...
        setRegData({ ...regData, email: loginEmail });
        setActiveTab('register');
        setWizardStep(3);
        setError('Please verify your account first. We have sent an OTP previously, or you can resend it.');
      } else {
  ...
  const handleSendOtp = async () => {
    ...
    setLoading(true);
    try {
      // The register endpoint sends the OTP as part of registration
      await axios.post(`${API_URL}/customer-auth/register`, regData);
      setOtpTimer(60);
      setWizardStep(3);
    } catch (err) {
  ...
  const handleResendOtp = async () => {
    ...
    setLoading(true);
    try {
      await axios.post(`${API_URL}/customer-auth/resend-otp`, { email: regData.email });
      setOtpTimer(60);
  ...
  const handleRegisterSubmit = async (e) => {
    ...
    if (wizardStep === 3) {
      ...
      setLoading(true);
      try {
        await axios.post(`${API_URL}/customer-auth/verify-otp`, { email: regData.email, otp: regData.otp });
